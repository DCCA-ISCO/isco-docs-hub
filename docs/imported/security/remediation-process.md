...
3. **Track** — Open findings are tracked in `docs/TODOS.md`. Each finding entry includes the Security Hub control ID, affected account, and the specific remediation action required.
...
6. **Close** — Mark the item in `docs/TODOS.md` as complete with the date resolved.
...
- Progress on findings tracked in `docs/TODOS.md`

The monthly review date and a brief summary of actions taken should be noted in the git commit history for `docs/TODOS.md`.
