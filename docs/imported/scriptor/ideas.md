...
23. **Light/dark mode toggle** — deferred post-UAT. Needs a semantic token layer (~3-5 days); see "Light / Dark Mode Toggle" entry above.
    - **Stopgap in place (2026-06-01):** office-lighting contrast was poor, so a blanket pass bumped all dark text to a readable floor — `text-slate-{500,600,700}` → `slate-400`, `placeholder-slate-600` → `slate-500` (commit 580f180), plus the transcript body to `slate-100`/`slate-300` (commit ed2ae6c). This **flattened the muted-text hierarchy** (500/600/700 now all render as 400). When the token layer lands, do NOT preserve this collapse: define semantic roles (e.g. `text-primary` ~slate-100/200, `text-muted` ~slate-400, `text-placeholder` ~slate-500) and re-introduce hierarchy by lifting *primary* text rather than dimming the muted end. The current floor is a readability band-aid, not the intended scale.
